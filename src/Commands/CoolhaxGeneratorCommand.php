...
use Coolhax\CoolhaxCrudGenerator\CoolhaxModelGenerator;
...
/**
 * Class CoolhaxGeneratorCommand.
 */
abstract class CoolhaxGeneratorCommand extends Command
{
    /**
    ...
    public function __construct(Filesystem $files)
    {
        ...
        $this->files = $files;
        $this->unwantedColumns = config('crud-bootstrap.model.unwantedColumns', $this->unwantedColumns);
        $this->modelNamespace = config('crud-bootstrap.model.namespace', $this->modelNamespace);
        $this->controllerNamespace = config('crud-bootstrap.controller.namespace', $this->controllerNamespace);
        $this->layout = config('crud-bootstrap.layout', $this->layout);
    }
    ...
    {
        $stub_path = config('crud-bootstrap.stub_path', 'default');
    ...
    protected function getField($title, $column, $type = 'form-field')
    {
        ...
        $replace = array_merge($this->buildReplacements(), [
            ...
            '{{selectTitle}}'  => $relationNameTitle,
        ]);

        // Check if the column has a relationship
        if ($type=='form-field' && $this->hasRelationship($column)) {
            // If a relationship exists, use the 'select-field' stub
            return str_replace(
                array_keys($replace), array_values($replace), $this->getStub("views/select-field")
            );
        } else {
            // If no relationship exists, use the 'input-field' stub or the default stub
            return str_replace(
    ...
    protected function hasRelationship($column)
    {
    ...
    protected function getBody($column)
    {
        if ($this->hasRelationship($column)) {
            $column = str_replace('_id','',$column)."->name";
        } 
        $replace = array_merge($this->buildReplacements(), [
            '{{column}}' => $column,
        ]);
    ...
    protected function modelReplacements()
    {
        ...
        list($relations, $properties) = (new CoolhaxModelGenerator($this->table, $properties, $this->modelNamespace))->getEloquentRelations();
